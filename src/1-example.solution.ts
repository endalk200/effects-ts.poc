...
function exercise2(): Effect.Effect<string, never, never> {
  return Effect.succeed("Hello, Effect!");
}
...
function exercise5(a: number, b: number): Effect.Effect<number, string, never> {
  if (b === 0) {
    return Effect.fail("Cannot modulo by zero");
  }
...
function exercise6(n: number): Effect.Effect<number, string, never> {
  if (n < 0) {
    return Effect.fail("Cannot take square root of negative number");
  }
...
function exercise11(n: number): Effect.Effect<number, never, never> {
  const result = exercise9(n).pipe(
    Effect.catchTag("ErrorNegativeNumber", (error) => {
      Effect.log("Error: ", error.message);
...
function exercise12(
  ...
  return Effect.succeed(Math.pow(base, exponent));
}
...
function exercise13(
  ...
  return Effect.succeed(Math.floor(a / b));
}
...
function exercise15(
  ...
  if (b === 0) {
    return Effect.fail(new ErrorDivisionByZero({ dividend: a }));
  }
...
function calculator(
  ...
  if (operation === "add") {
    ...
    if (b === 0) return Effect.fail(new ErrorDivisionByZero({ dividend: a }));

    return Effect.succeed(a / b);
  } else if (operation === "multiply") {
    return Effect.succeed(a * b);
  } else {
    return Effect.fail(new ErrorInvalidOperation({ operation: operation }));
  }
}
